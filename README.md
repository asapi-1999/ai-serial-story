...
GitHub Actions (毎週月曜)
   └─ generate.py が Gemini API を呼び出して続きを生成
        ...
        ├─ bible.json   … 連載中の作品の設定・文体指針・全話構成・人物状態
...
同時にmainを更新した場合は、既存の生成元SHA検証により小説生成側が安全に停止します。
...
- 第1話でジャンル、テーマ、再現可能な文体指針、全話の構成案を作り、`bible.json` に保存します。
- 続きの生成時は **bible.json の設定（作品タイトル・世界観・登場人物・文体指針・全話構成・
  未回収要素・人物状態・全話あらすじ）＋直近2話の全文**
  を文脈として渡します。全話の本文を毎回渡さないので、連載が長くてもコストと文脈上限を抑えられます。
- ジャンル・テーマ・文体・全話構成は作品中に固定し、未回収要素と人物状態だけを各話終了時点へ更新します。
  旧形式の `bible.json` はそのまま利用でき、次回生成時に既存本文から新しい管理情報を自動補完します。

### 構造化出力とデータ保護

- Geminiには第1話用／続話用のJSON Schemaを指定し、タイトル・本文・あらすじ・人物・シリーズ管理情報を
  構造化JSONで受け取ります。
